...
/* beacon enconder */
static int32_t encoder_val = 0;
static int16_t encoder_val_previous = 0;
...
void beacon_start(void)
{
    ...
    cs_set_consign(&mainboard.beacon_speed.cs, 15);
}
...
*/
    ...
    if(sensor == IR_SENSOR_180_DEG)
        local_angle = get_angle(count_middle, local_count_period, (90-8));
    else
...
#define PLAYGROUND_MARGIN 120
    if(result_x < PLAYGROUND_MARGIN || result_x > (3000-PLAYGROUND_MARGIN)) {
        //XXX BEACON_NOTICE("discard xy (%ld %ld), x is out of playground", result_x, result_y);
        //goto error;
    }
    if(result_y < PLAYGROUND_MARGIN || result_y > (2000-PLAYGROUND_MARGIN)) {
        //XXX BEACON_NOTICE("discard xy (%ld %ld), y is out of playground", result_x, result_y);
        //goto error;
    }
    ...
    if(sensor == IR_SENSOR_180_DEG)
    {
		...
		beaconboard.opponent1_x = beacon.opponent1_x;
		beaconboard.opponent1_y = beacon.opponent1_y;
		beaconboard.opponent1_a = beacon.opponent1_a;
		beaconboard.opponent1_d = beacon.opponent1_d;
        IRQ_UNLOCK(flags);
...
#define TRACKING_WINDOW_mm    200
        ...
        if(tracking_update == OPPONENT_1) {
			...
			beaconboard.opponent1_x = beacon.opponent1_x;
			beaconboard.opponent1_y = beacon.opponent1_y;
			beaconboard.opponent1_a = beacon.opponent1_angle;
			beaconboard.opponent1_d = beacon.opponent1_dist;
            IRQ_UNLOCK(flags);
			...
			beaconboard.opponent2_x = beacon.opponent2_x;
			beaconboard.opponent2_y = beacon.opponent2_y;
			beaconboard.opponent2_a = beacon.opponent2_angle;
			beaconboard.opponent2_d = beacon.opponent2_dist;
            IRQ_UNLOCK(flags);
